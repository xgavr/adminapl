...
class PaymentManager 
{
    ...
    public function addPayment($data)
    {
        ...
        $payment->setTaxInfoDocumentDate(empty($data['taxInfoDocumentDate']) ? 0:$data['taxInfoDocumentDate']);
        $payment->setTaxInfoDocumentNumber(empty($data['taxInfoDocumentNumber']) ? 0:$data['taxInfoDocumentNumber']);
        $payment->setTaxInfoKbk(empty($data['taxInfoKbk']) ? 0:$data['taxInfoKbk']);
        $payment->setTaxInfoOkato(empty($data['taxInfoOkato']) ? null:$data['taxInfoOkato']);
        $payment->setTaxInfoPeriod(empty($data['taxInfoPeriod']) ? 0:$data['taxInfoPeriod']);
        $payment->setTaxInfoReasonCode(empty($data['taxInfoReasonCode']) ? 0:$data['taxInfoReasonCode']);
        $payment->setTaxInfoStatus(empty($data['taxInfoStatus']) ? Payment::TAX_STATUS_01:$data['taxInfoStatus']);
