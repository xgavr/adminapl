    ...
    public function addGroupTokenFromGood($goodId, $oldTokenGroupId = null)
    {
        ...
        if (is_array($groupTitles)){
            if (count($groupTitles)){
                ...
                $titleTokenCount0 = count(explode('_', $groupTitle0['tokenGroupTitle']));
                
                //выбор названия, где больше токенов
                foreach ($groupTitles as $groupTitle){
                    $titleTokenCount = count(explode('_', $groupTitle['tokenGroupTitle']));
                    if ($titleTokenCount > $titleTokenCount0){
                        $titleTokenCount0 = $titleTokenCount;
                        $groupTitle0 = $groupTitle;
                    }
                }                                
    ...
    public function aiNaming($good)
    {
        if (count($good->articleTitles())){
            $messages[] = [
                ...
                . 'товара из прайсов поставщиков. Сделай из них для карточки товара:'.PHP_EOL
                . '1. Наименование (не более 90 знаков) по следующему правилу: '
                . 'Тип товара (обязательно) + Производитель (обязательно) + Артикул (обязательно) + Применимость (если есть) + Особенности (объем/кол-во/цвет/сезонность/марка авто и пр),'.PHP_EOL
                . '2. Описание (до 900 знаков).'.PHP_EOL
                . 'Результат верни в формате '
                . 'JSON без каких-либо пояснений, например, '
