...
//                        $result[$token->getIdf().'_'.$token->getId()] = $token;
                ...
                if ($k > 0){
                    ...
                    if ($bigram){
                        if (in_array($bigram->getStatus(), [Bigram::RU_RU, Bigram::RU_EN, Bigram::RU_NUM])){
                            $pmi = log($bigram->getFrequency()*$gc*$bigram->getFlag()/($preToken->getFrequency()*$token->getFrequency()));
                            if ($pmi < 0 
        ...
        if ($k == 0 && $token){
            ...
            if ($bigram){
                if (in_array($bigram->getStatus(), [Bigram::RU_RU, Bigram::RU_EN, Bigram::RU_NUM])){
                    $pmi = log(($bigram->getFrequency()*$gc*$bigram->getFlag())/($token->getFrequency()*2));
                    if ($pmi < 0 
                            || $bigram->getFlag() != Bigram::WHITE_LIST){
                        $pmi = 0;
                    }
